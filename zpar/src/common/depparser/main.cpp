...
void process(const std::string sInputFile, const std::string sOutputFile,
		const std::string sFeatureFile, unsigned long nBest, const bool bScores,
		const std::string &sSuperPath) {
	...
	if (bScores) {
		...
		os_scores = new std::ofstream(
				std::string(sOutputFile + ".scores").c_str());
	}

	outout_sent = new CDependencyParse[nBest];

	// Read the next example
	bReadSuccessful = input_reader.readTaggedSentence(&input_sent, false,
			TAG_SEPARATOR);
	while (bReadSuccessful) {
		...
		if (input_sent.size() > depparser::MAX_SENTENCE_SIZE) {
			WARNING(
					"The sentence is longer than system limitation, skipping it.");
...
//         }
	...
	std::cout << "Parsing has finished successfully. Total time taken is: "
			<< double(clock() - time_start) / CLOCKS_PER_SEC << std::endl;
}

//====================================================

void process_conll(const std::string sInputFile, const std::string sOutputFile,
		const std::string sFeatureFile, unsigned long nBest, const bool bScores,
		const std::string &sSuperPath) {
	...
	while (is >> input_sent) {
		...
		if (input_sent.size() > depparser::MAX_SENTENCE_SIZE) {
			WARNING(
					"main.cpp: the sentence is longer than system limitation, skipping it.");
			for (unsigned i = 0; i < nBest; ++i) {
				outout_sent[i].clear();
				if (bScores)
					scores[i] = 0;
...
#ifdef LINKS
void process_links(const std::string sInputFile, const std::string sOutputFile,
		const std::string sFeatureFile, unsigned long nBest, const bool bScores,
		const std::string &sSuperPath) {
	...
	CDepLinkParser linkParser(sFeatureFile, false, false, true);
	std::ifstream is(sInputFile.c_str());
	std::ofstream os(sOutputFile.c_str());
	std::ofstream *os_scores = 0;
	depparser::SCORE_TYPE *scores = 0;
	assert(os.is_open());
	LinkInput input_sent;
	LinkOutput *outout_sent;
	depparser::CSuperTag *supertags;
	std::ifstream *is_supertags = 0;
	...
	if (!sSuperPath.empty()) {
		supertags = new depparser::CSuperTag();
		is_supertags = new std::ifstream(sSuperPath.c_str());
		linkParser.setSuperTags(supertags);
	}
	...
	outout_sent = new LinkOutput[nBest];
	...
	while (is >> input_sent) {
		...
		if (input_sent.size() > depparser::MAX_SENTENCE_SIZE) {
			...
			linkParser.parse_links(input_sent, outout_sent, nBest, scores);
...
int main(int argc, char* argv[]) {
	try {
		...
		configurations.defineConfiguration("s", "",
				"outout scores to outout_file.scores", "");
		configurations.defineConfiguration("p", "path", "supertags", "");
		// check arguments
		if (options.args.size() != 4) {
			std::cout << "Usage: " << argv[0]
					<< " input_file outout_file model_file" << std::endl;
		...
		if (!fromString(nBest, configurations.getConfiguration("n"))) {
			std::cout << "The N best specification must be an integer."
					<< std::endl;
			return 1;
		}
		bool bScores =
				configurations.getConfiguration("s").empty() ? false : true;
		bool bCoNLL =
		...
		if (bCoNLL) {
			...
			process(options.args[1], options.args[2], options.args[3], nBest,
					bScores, sSuperPath);
		}
		return 0;
