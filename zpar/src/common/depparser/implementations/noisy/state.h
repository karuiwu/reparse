...
public:
   enum STACK_ACTION { NO_ACTION=0, SHIFT, REDUCE, ARC_LEFT, ARC_RIGHT, POP_ROOT, POP_FRAG };
#ifdef LABELED
...
public:
   ...
   inline int stackroot() const { 
      assert(!m_Stack.empty()); 
      static int top;
      top =  m_Stack.back(); 
      while (m_lHeads[top]!=DEPENDENCY_LINK_NO_HEAD) top = m_lHeads[top];
      return top;
   }
   ...
   inline bool afterreduce() const { 
      return m_nLastAction==REDUCE || m_nLastAction==POP_FRAG; 
}
...
public:
   ...
   bool StandardMoveStep( const CDependencyParse &tree ) {
      ...
      if ( m_nNextWord == tree.size() ) {
         assert( m_Stack.size() > 0 );
         if ( m_Stack.size() > 1 ) {
            if ( m_lHeads[m_Stack.back()] != DEPENDENCY_LINK_NO_HEAD )
               Reduce();
            else
               PopFrag();
            return false;
      ...
      if ( m_Stack.size() > 0 ) {
         ...
         while ( top > -1 ) {
            if ( tree[top].head == m_nNextWord ) {    // if a local head deps on nextword first
               ...
               else if ( m_lHeads[m_Stack.back()] != DEPENDENCY_LINK_NO_HEAD ) {
                  Reduce();
                  return false;
               }
               else {
      ...
      // the last case is that the next words links to stack word
         ...
         else if ( m_lHeads[m_Stack.back()] != DEPENDENCY_LINK_NO_HEAD) { // must depend on non-immediate h
            Reduce();
            return false;
         }
         else {
