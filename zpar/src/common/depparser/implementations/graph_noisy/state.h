...
public:
   ...
   // NOISY make fragmented span
   void setFragmentedSpan(const CSpan &left, const CSpan &right, SPAN_TYPE type) {
      assert(left.m_nRight == right.m_nLeft);
      assert(left.isActive() && right.isActive());
      assert(left.isMinimal());
      setConcatenateSpan(left, right);
      m_nType = type;
      if (left.m_nType == BF && right.m_nType == BF && type != BF)
         addCoveringLink(type);
   }
   // clear a span
   inline void clear() { 
