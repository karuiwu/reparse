...
public class Engine  {
	...
	public FlowChartInstance initialize(int optionContainerIndex) throws MaltChainedException {
		...
		if (OptionManager.instance().getOptionValueNoDefault(optionContainerIndex, "config", "flowchart") != null) {//JUNEKI: gets all of the option stuff. Including our talbanken file address.
	...
	public void process(int optionContainerIndex) throws MaltChainedException {
		...
		//System.err.println("JUNEKI: optionContainerIndex " + optionContainerIndex);
		//System.err.println("JUNEKI: flowChartInstances " + flowChartInstances);
		
		FlowChartInstance flowChartInstance = flowChartInstances.get(optionContainerIndex); // Gets everything from the last step
		
		
		System.err.println("JUNEKI: flowChartInstance " + flowChartInstance);
		...
		if (flowChartInstance.hasProcessChartItems()) {
			...
			while (signal != ChartItem.TERMINATE) {
				...
				//This here. Decisions are made within here.
				signal = flowChartInstance.process();
				
				//System.err.println("JUNEKI: signal " + signal);
				
				if (signal == ChartItem.CONTINUE) {
					sentenceCounter++;
					
					System.out.println();
					System.out.println("(Engine.java, line 81) JUNEKI: sentenceCounter " + sentenceCounter);
					System.out.println();
					System.out.println();
					...
					//System.err.println("JUNEKI: nIteration "+nIteration);
