...
void clearPointerQueue(priority_queue<T*,std::vector<T*>,C >& q) {
  while(q.size() > 0) {
    T* s = q.top();
    q.pop();
    delete s;
  }
  q = priority_queue<T*,std::vector<T*>,C >();
}

class StopWatchClock {
public:
  StopWatchClock() : start_(true){};
  double push() {
    if (start_) {
      start_time_ = clock();
      start_ = false;
    ...
    end_time_ = clock();
    double elapse_usec = (double) (end_time_ - start_time_);
    start_ = true;
    return elapse_usec;
...
inline int Tokenize( string str, vector<string> &tokens, string sep = " " ) {
  int p = 0;
  int p2 = 0;
